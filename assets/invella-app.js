...
var currentCoverPath='',coverPreviewUrl='',currentMusicPath='',musicPreviewUrl='',currentGalleryPaths=[],galleryPreviewUrls=[],countdownTimer=null,pendingCoverFile=null,pendingMusicFile=null,pendingGalleryFiles=[],coverUploadPromise=null,musicUploadPromise=null,galleryUploadPromise=null;
var fields=['eventType','customEvent','names','date','time','venue','address','message','p1t','p1','p2t','p2','p3t','p3','dress','wishes','childrenPolicy','backgroundStyle','backgroundCustomUrl','backgroundCustomPath','invFont','c1','c2','c3','c4'];
...
function enforceEditorPlan(){var paid=currentEditorPlan&&currentEditorPlan!=='draft';var pro=editorHas('pro'),premium=editorHas('premium');if(paid&&!pro){if($('backgroundStyle'))$('backgroundStyle').value='auto';if($('showRSVP'))$('showRSVP').checked=false}if(paid&&!premium&&$('backgroundStyle')&&$('backgroundStyle').value==='custom')$('backgroundStyle').value=pro?'silk':'auto';document.querySelectorAll('#bgPicker .bg-choice').forEach(function(b){var need=b.dataset.bg==='auto'?'basic':'pro';b.disabled=paid&&!editorHas(need);b.classList.toggle('plan-locked',b.disabled)});if($('customBackgroundFile'))$('customBackgroundFile').disabled=paid&&!premium;if($('photo'))$('photo').disabled=paid&&!pro;if($('guestNamesForLinks'))$('guestNamesForLinks').disabled=!premium;if($('invFont'))$('invFont').disabled=paid&&!pro;var gl=$('guestNamesForLinks')&&$('guestNamesForLinks').closest('.feature-box');if(gl){var b=gl.querySelector('button');if(b)b.disabled=!premium}if($('showRSVP'))$('showRSVP').disabled=paid&&!pro;['premiumMusic','premiumGallery','premiumTimer'].forEach(function(id){if($(id))$(id).disabled=paid&&!premium})}
...
await loadMediaState(r.data);
render();
setEditorAction(r.data.status==='published');
}catch(e){console.error('Invella restore:',e)}
}

function collectData(){var d={};fields.forEach(function(k){if($(k))d[k]=$(k).value});['showProgram','showVenue','showDress','showWishes','showRSVP'].forEach(function(k){if($(k))d[k]=$(k).checked});d.showPremiumTimer=!!($('premiumTimer')&&$('premiumTimer').checked);return d}
...
async function cloudSave(){if(!currentUser)return false;var data=collectData(),safeCover=currentCoverPath&&!/^(blob:|data:)/i.test(currentCoverPath)?currentCoverPath:null,safeMusic=currentMusicPath&&!/^(blob:|data:)/i.test(currentMusicPath)?currentMusicPath:null,safeGallery=currentGalleryPaths.filter(function(x){return typeof x==='string'&&!/^(blob:|data:)/i.test(x)}),payload={owner_id:currentUser.id,title:$('names').value||'Новое приглашение',event_date:eventISO(),venue_name:$('venue').value,venue_address:$('address').value,template_key:template,content:data,design:{colors:[data.c1,data.c2,data.c3,data.c4]},cover_url:safeCover,music_url:safeMusic,gallery:safeGallery};try{if(currentInvitationId){var u=await sb.from('invitations').update(payload).eq('id',currentInvitationId).eq('owner_id',currentUser.id).select('id,slug').maybeSingle();if(u.error)throw u.error;if(!u.data)throw new Error('Приглашение не найдено или нет права на редактирование.');currentInvitationSlug=u.data.slug;return true}payload.slug=makeSlug();var r=await sb.from('invitations').insert(payload).select('id,slug').single();if(r.error)throw r.error;currentInvitationId=r.data.id;currentInvitationSlug=r.data.slug;safeSet('invella_cloud_id_'+currentUser.id,r.data.id);safeSet('invella_cloud_slug_'+currentUser.id,r.data.slug);return true}catch(e){console.error('Invella save:',e);return false}}
...
var EVENT_PRESETS={
'Свадьба':['Будем счастливы разделить с вами один из самых важных дней нашей жизни.',['Сбор гостей','Церемония','Праздничный ужин'],'Будем рады, если вы поддержите палитру нашего праздника.','Ваше присутствие — лучший подарок для нас.'],
'День рождения':['Приглашаю вас провести этот день вместе со мной — будет тепло, весело и по-настоящему празднично!',['Сбор гостей','Праздничная программа','Торт и поздравления'],'Выбирайте праздничный образ, в котором вам будет комфортно.','Главное — ваше хорошее настроение и желание праздновать вместе.'],
'Юбилей':['Приглашаем вас разделить с нами этот особенный день и провести его в кругу дорогих людей.',['Сбор гостей','Праздничный ужин','Поздравления и программа'],'Будем рады нарядным вечерним образам.','Самое важное — разделить этот вечер вместе.'],
'Помолвка':['Мы сказали друг другу «да» и хотим разделить радость этого события с вами.',['Сбор гостей','Праздничный ужин','Тосты и поздравления'],'Будем рады элегантным праздничным образам.','Ваше присутствие сделает этот вечер ещё теплее.'],
'Годовщина':['Приглашаем вас отметить вместе с нами ещё одну важную дату нашей истории.',['Сбор гостей','Праздничный ужин','Тосты и воспоминания'],'Выбирайте праздничный образ, в котором вам будет комфортно.','Будем счастливы провести этот вечер вместе с вами.'],
'Baby shower':['Совсем скоро нас станет больше! Приглашаем вас на тёплый праздник в ожидании малыша.',['Сбор гостей','Игры и общение','Праздничное угощение'],'Нежные светлые оттенки особенно приветствуются.','Приходите с хорошим настроением и тёплыми пожеланиями для малыша.'],
'Гендер-пати':['Приглашаем вас вместе с нами узнать главный секрет — кто же у нас будет?',['Сбор гостей','Игры и общение','Главный сюрприз'],'Можно поддержать интригу образом в розовых, голубых или нейтральных оттенках.','Берите с собой хорошее настроение и свою версию — мальчик или девочка?'],
'Выпускной':['Приглашаем вас отметить окончание важного этапа и начало новой главы!',['Сбор гостей','Торжественная часть','Праздничная программа'],'Дресс-код — праздничный.','Берите с собой настроение праздновать, танцевать и запоминать этот вечер.'],
'Корпоратив':['Коллеги, приглашаем вас провести этот вечер вместе, отдохнуть от рабочих задач и отлично провести время!',['Сбор гостей','Приветственная часть','Развлекательная программа'],'Дресс-код мероприятия — праздничный.','Берите с собой отличное настроение — всё остальное уже подготовлено.'],
'Вечеринка':['Собираем своих людей в одном месте — приходите отдыхать, общаться и танцевать!',['Сбор гостей','Старт вечеринки','Музыка и танцы'],'Выбирайте образ под настроение вечеринки.','Хорошее настроение обязательно. Остальное — по желанию.'],
'Другое событие':['Будем рады разделить это событие вместе с вами.',['Сбор гостей','Начало мероприятия','Праздничная программа'],'Выбирайте образ, соответствующий формату вашего события.','Будем рады видеть вас и провести этот день вместе.']
};
function isWedding(){return !!($('eventType')&&$('eventType').value==='Свадьба')}
function applyEventContext(){var wedding=isWedding(),cp=$('childrenPolicy');if(cp){var wrap=cp.closest('.feature-box');if(wrap)wrap.style.display=wedding?'':'none';if(!wedding)cp.value='unspecified'}var flowers=document.querySelector('.wish-presets button[onclick*="flowers"]');if(flowers)flowers.style.display=wedding?'':'none';var wishes=$('wishes');if(wishes){var wf=wishes.closest('.field');if(wf)wf.style.display=''}if($('blockChildren'))$('blockChildren').classList.toggle('hidden',!wedding||!cp||cp.value!=='adults_only');if($('childQuestion'))$('childQuestion').classList.toggle('hidden',!wedding||!cp||cp.value!=='welcome');var nf=$('names')&&$('names').closest('.field'),nl=nf&&nf.querySelector('label');if(nl)nl.textContent=wedding?'Имена':'Имя / название праздника'}
function applyEventPreset(){var p=EVENT_PRESETS[$('eventType').value]||EVENT_PRESETS['Другое событие'];$('message').value=p[0];$('p1').value=p[1][0];$('p2').value=p[1][1];$('p3').value=p[1][2];$('dress').value=p[2];$('wishes').value=p[3];applyEventContext();render()}
function renderNames(){var n=$('vNames'),raw=$('names').value.trim()||'Ваш праздник';if(!n)return;if(isWedding()){var parts=raw.split(/\s*(?:&|\+|\sи\s)\s*/i).filter(Boolean);if(parts.length===2){n.innerHTML='<span>'+escapeHtml(parts[0])+'</span><span class="wedding-divider">❦</span><span>'+escapeHtml(parts[1])+'</span>';return}}n.textContent=raw}
function applyFont(){var f=$('invFont'),family=f&&f.value||'Georgia,serif',inv=document.querySelector('.invitation'),n=$('vNames');if(inv)inv.style.setProperty('--inv-font',family);if(n)n.style.fontFamily=family}
function clearMediaPreview(){['premiumGalleryPreview','premiumCountdown','premiumAudio','premiumMusicToggle'].forEach(function(id){var e=$(id);if(e)e.remove()});if(countdownTimer){clearInterval(countdownTimer);countdownTimer=null}}
function countdownText(){var date=$('date')&&$('date').value,time=$('time')&&$('time').value||'00:00';if(!date)return'Укажите дату события';var diff=Math.max(0,new Date(date+'T'+time+':00').getTime()-Date.now()),days=Math.floor(diff/86400000),hours=Math.floor(diff%86400000/3600000),mins=Math.floor(diff%3600000/60000),secs=Math.floor(diff%60000/1000);return days+' дн. '+hours+' ч. '+mins+' мин. '+secs+' сек.'}
function renderPremiumMedia(){var inv=document.querySelector('.invitation');if(!inv)return;clearMediaPreview();var premium=currentPublicPlan!==null?currentPublicPlan==='premium':(currentEditorPlan==='draft'||editorHas('premium'));if(!premium)return;var intro=$('guestHeading')&&$('guestHeading').closest('.block'),anchor=intro||$('invHero');if(currentGalleryPaths.length||galleryPreviewUrls.length){var urls=galleryPreviewUrls.length?galleryPreviewUrls:currentGalleryPaths;if(urls.length){var g=document.createElement('section');g.id='premiumGalleryPreview';g.className='block premium-gallery';g.innerHTML='<div class="eyebrow">ФОТОГАЛЕРЕЯ</div><div class="premium-gallery-grid">'+urls.slice(0,5).map(function(u){return '<button type="button" class="premium-gallery-photo" aria-label="Открыть фотографию"><img src="'+escapeHtml(u)+'" alt="Фотография события"></button>'}).join('')+'</div>';g.querySelectorAll('button').forEach(function(b){b.onclick=function(){window.open(b.querySelector('img').src,'_blank')}});anchor.insertAdjacentElement('afterend',g);anchor=g}}
if($('premiumTimer')&&$('premiumTimer').checked){var t=document.createElement('section');t.id='premiumCountdown';t.className='block premium-countdown';t.innerHTML='<div class="eyebrow">ДО СОБЫТИЯ</div><div id="premiumCountdownValue"></div>';anchor.insertAdjacentElement('afterend',t);var tick=function(){if($('premiumCountdownValue'))$('premiumCountdownValue').textContent=countdownText()};tick();countdownTimer=setInterval(tick,1000)}
var music=musicPreviewUrl||currentMusicPath;if(music){var a=document.createElement('audio');a.id='premiumAudio';a.src=music;a.loop=true;a.preload='metadata';var b=document.createElement('button');b.id='premiumMusicToggle';b.type='button';b.className='premium-music-toggle';b.textContent='♪';b.setAttribute('aria-label','Включить или выключить музыку');b.onclick=function(){if(a.paused)a.play().then(function(){b.classList.add('is-playing')}).catch(function(){});else{a.pause();b.classList.remove('is-playing')}};inv.appendChild(a);inv.appendChild(b);a.play().then(function(){b.classList.add('is-playing')}).catch(function(){})}}
async function loadMediaState(row){var c=row.content||{};currentCoverPath=row.cover_url||'';currentMusicPath=row.music_url||c.premiumMusicUrl||'';currentGalleryPaths=Array.isArray(row.gallery)&&row.gallery.length?row.gallery:(Array.isArray(c.premiumGallery)?c.premiumGallery:[]);coverPreviewUrl=await signedAssetUrl(currentCoverPath);musicPreviewUrl=await signedAssetUrl(currentMusicPath);galleryPreviewUrls=(await Promise.all(currentGalleryPaths.slice(0,5).map(signedAssetUrl))).filter(Boolean);if($('premiumTimer'))$('premiumTimer').checked=!!(c.showPremiumTimer||c.premiumTimer);applyFont();renderPremiumMedia()}
function applyCover(){var h=$('invHero'),u=coverPreviewUrl||(/^https?:|^blob:|^data:/.test(currentCoverPath)?currentCoverPath:'');if(!h||!u)return;h.style.backgroundImage='linear-gradient(rgba(0,0,0,.28),rgba(0,0,0,.28)),url("'+String(u).replace(/"/g,'%22')+'")';h.style.backgroundSize='cover';h.style.backgroundPosition='center';h.style.color='#fff'}
window.editCloudInvitation=async function(id){var r=await sb.from('invitations').select('*').eq('id',id).eq('owner_id',currentUser.id).single();if(r.error||!r.data)return alert('Не удалось открыть приглашение.');currentInvitationId=r.data.id;currentInvitationSlug=r.data.slug;currentEditorPlan=String(r.data.plan||'draft').toLowerCase();template=r.data.template_key||'classic';var d=r.data.content||{};fields.forEach(function(k){if($(k)&&d[k]!=null)$(k).value=d[k]});['showProgram','showVenue','showDress','showWishes','showRSVP'].forEach(function(k){if($(k)&&d[k]!=null)$(k).checked=Boolean(d[k])});await loadMediaState(r.data);closeDashboard();$('marketing').classList.add('hidden');$('app').classList.remove('hidden');history.pushState({view:'editor',id:id},'','/create/editor');render();setEditorAction(r.data.status==='published');scrollTo(0,0)}
function setEditorAction(published){var b=document.querySelector('.appbar .toolbar .primary');if(!b)return;if(published){b.textContent='Сохранить изменения';b.onclick=window.invellaSavePublished}else{b.textContent='Опубликовать';b.onclick=window.publishDemo}}
window.invellaSavePublished=async function(){var b=document.querySelector('.appbar .toolbar .primary');if(!currentInvitationId)return alert('Приглашение не выбрано.');if(b){b.disabled=true;b.textContent='Сохраняем файлы…'}try{await Promise.all([coverUploadPromise,musicUploadPromise,galleryUploadPromise].filter(Boolean));if(b)b.textContent='Сохраняем…';var ok=await cloudSave();if(!ok)throw new Error('Не удалось обновить запись.');alert('Изменения сохранены')}catch(e){alert(e.message||'Не удалось сохранить изменения.')}finally{if(b){b.disabled=false;b.textContent='Сохранить изменения'}}}
...
currentCoverPath=x.cover_url||'';
coverPreviewUrl=currentCoverPath;
currentMusicPath=currentPublicPlan==='premium'?(x.music_url||''):'';
musicPreviewUrl=currentMusicPath;
currentGalleryPaths=currentPublicPlan==='premium'&&Array.isArray(x.gallery)?x.gallery:[];
galleryPreviewUrls=currentGalleryPaths.slice(0,5);
if($('premiumTimer'))$('premiumTimer').checked=currentPublicPlan==='premium'&&!!(d.showPremiumTimer||d.premiumTimer);
...
if(guestParam&&currentPublicPlan==='premium'){
...
function resetPublicLayout(){var controls=document.querySelector('.controls'),bar=document.querySelector('.appbar'),editor=document.querySelector('.editor');if(controls)controls.classList.remove('hidden');if(bar)bar.classList.remove('hidden');if(editor)editor.style.gridTemplateColumns='';currentInvitationId=null;currentInvitationSlug=null;currentPublicPlan=null;currentEditorPlan='draft';currentCoverPath='';coverPreviewUrl='';currentMusicPath='';musicPreviewUrl='';currentGalleryPaths=[];galleryPreviewUrls=[];setEditorAction(false);clearMediaPreview()}
...
function load(){template=safeGet('invella_template')||template;var raw=safeGet('invella_draft');if(!raw)return;try{var d=JSON.parse(raw);fields.forEach(function(k){if($(k)&&d[k]!=null)$(k).value=d[k]});['showProgram','showVenue','showDress','showWishes','showRSVP'].forEach(function(k){if($(k)&&d[k]!=null)$(k).checked=!!d[k]});if($('premiumTimer')&&d.showPremiumTimer!=null)$('premiumTimer').checked=!!d.showPremiumTimer}catch(e){}}
...
function applyTemplate(){var h=$('invHero'),inv=document.querySelector('.invitation'),map={classic:['#e8ded4','#201d1b'],editorial:['#f4eee5','#201d1b'],dark:['#20201e','#fff'],botanical:['#e6eadf','#201d1b'],romance:['#eadce0','#201d1b'],modern:['#dce4e7','#201d1b']},x=map[template]||map.classic;h.style.backgroundImage='none';h.style.backgroundColor=x[0];h.style.color=x[1];if(inv){inv.className='invitation theme-'+template}applyInvitationBackground();applyCover();applyFont()}
...
function render(){enforceEditorPlan();toggleCustomEvent();applyEventContext();var eventLabel=$('eventType').value==='Другое событие'?($('customEvent').value.trim()||'Ваше событие'):$('eventType').value;setText('vEventType',eventLabel||'Ваш праздник');renderNames();setText('vDate',dateText());setText('vMessage',$('message').value);setText('vVenue',$('venue').value);setText('vAddress',$('address').value);setText('vDress',$('dress').value);setText('vWishes',$('wishes').value);['1','2','3'].forEach(function(n){setText('vp'+n+'t',$('p'+n+'t').value);setText('vp'+n,$('p'+n).value)});['1','2','3','4'].forEach(function(n){$('d'+n).style.backgroundColor=$('c'+n).value});setBlockVisible('blockProgram','showProgram');setBlockVisible('blockVenue','showVenue');setBlockVisible('blockDress','showDress');setBlockVisible('blockWishes','showWishes');var publicMode=isPublicPath()&&currentPublicPlan!==null;var canUseRSVP=!publicMode||['pro','premium'].includes(currentPublicPlan);var rsvp=$('blockRSVP');if(rsvp)rsvp.classList.toggle('hidden',!canUseRSVP||!$('showRSVP').checked);applyTemplate();if(!publicMode)save()}
...
if(!editorHas('premium'))return alert('Персональные ссылки доступны только в Premium.');
...
if($('eventType'))$('eventType').addEventListener('change',applyEventPreset);
$('photo').addEventListener('change',async function(e){var original=e.target.files[0];if(!original)return;pendingCoverFile=original;coverPreviewUrl=URL.createObjectURL(original);applyTemplate();coverUploadPromise=(async function(){try{var f=await compressImage(original);if(!currentUser)return;if(!currentInvitationId&&!(await cloudSave()))throw new Error('Не удалось подготовить приглашение.');await uploadCover(f);if(!(await cloudSave()))throw new Error('Не удалось сохранить обложку.');pendingCoverFile=null}catch(err){console.error('Invella cover upload:',err);alert(err.message||'Не удалось загрузить обложку.')}finally{coverUploadPromise=null}})();await coverUploadPromise});
if($('premiumTimer'))$('premiumTimer').addEventListener('change',function(){renderPremiumMedia();save()});
if($('premiumMusic'))$('premiumMusic').addEventListener('change',async function(e){var f=e.target.files&&e.target.files[0];if(!f)return;if(f.size>26214400){e.target.value='';return alert('Аудиофайл слишком большой. Максимальный размер — 25 МБ.')}pendingMusicFile=f;currentMusicPath='';musicPreviewUrl=URL.createObjectURL(f);renderPremiumMedia();if(!currentUser)return;musicUploadPromise=(async function(){try{if(!currentInvitationId&&!(await cloudSave()))throw new Error('Не удалось подготовить приглашение.');var path=await uploadAsset(f,'music');if(!path)throw new Error('Файл не был загружен.');currentMusicPath=path;musicPreviewUrl=await signedAssetUrl(path);if(!(await cloudSave()))throw new Error('Не удалось сохранить музыку.');pendingMusicFile=null;renderPremiumMedia()}catch(err){console.error('Invella music upload:',err);alert('Музыка не загрузилась: '+(err.message||'проверьте формат и размер файла.'));throw err}finally{musicUploadPromise=null}})();try{await musicUploadPromise}catch(err){}});
if($('premiumGallery'))$('premiumGallery').addEventListener('change',async function(e){var fs=Array.from(e.target.files||[]).slice(0,5);pendingGalleryFiles=fs;galleryPreviewUrls=fs.map(function(f){return URL.createObjectURL(f)});renderPremiumMedia();if(!currentUser||!fs.length)return;galleryUploadPromise=(async function(){try{if(!currentInvitationId&&!(await cloudSave()))throw new Error('Не удалось подготовить приглашение.');var paths=[];for(var i=0;i<fs.length;i++)paths.push(await uploadAsset(await compressImage(fs[i]),'gallery',i));currentGalleryPaths=paths;galleryPreviewUrls=(await Promise.all(paths.map(signedAssetUrl))).filter(Boolean);if(!(await cloudSave()))throw new Error('Не удалось сохранить галерею.');pendingGalleryFiles=[];renderPremiumMedia()}catch(err){console.error('Invella gallery upload:',err);alert(err.message||'Не удалось загрузить галерею.');throw err}finally{galleryUploadPromise=null}})();try{await galleryUploadPromise}catch(err){}});
