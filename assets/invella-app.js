...
function enforceEditorPlan(){var paid=currentEditorPlan&&currentEditorPlan!=='draft';var pro=editorHas('pro'),premium=editorHas('premium');if(paid&&!pro){if($('backgroundStyle'))$('backgroundStyle').value='auto';if($('showRSVP'))$('showRSVP').checked=false}if(paid&&!premium&&$('backgroundStyle')&&$('backgroundStyle').value==='custom')$('backgroundStyle').value=pro?'silk':'auto';document.querySelectorAll('#bgPicker .bg-choice').forEach(function(b){var need=b.dataset.bg==='auto'?'basic':'pro';b.disabled=paid&&!editorHas(need);b.classList.toggle('plan-locked',b.disabled)});if($('customBackgroundFile'))$('customBackgroundFile').disabled=paid&&!premium;if($('photo'))$('photo').disabled=paid&&!pro;if($('guestNamesForLinks'))$('guestNamesForLinks').disabled=paid&&!pro;var gl=$('guestNamesForLinks')&&$('guestNamesForLinks').parentElement&&$('guestNamesForLinks').parentElement.parentElement;if(gl){var b=gl.querySelector('button');if(b)b.disabled=paid&&!pro}if($('showRSVP'))$('showRSVP').disabled=paid&&!pro;initPremium();}
function initPremium(){var m=$("premiumMusic"),g=$("premiumGallery"),t=$("premiumTimer");[m,g,t].forEach(function(x){if(x)x.disabled=!editorHas("premium")});if(t)t.onchange=function(){safeSet("premium_timer",this.checked?"1":"0");scheduleCloudSave()};if(m)m.onchange=function(){if(this.files&&this.files[0])alert("Музыка выбрана: "+this.files[0].name)};if(g)g.onchange=function(){alert("Выбрано фотографий: "+(this.files||[]).length)}}
...
async function performCloudSave(){if(!currentUser)return false;try{var existing={};if(currentInvitationId){var old=await sb.from('invitations').select('content,cover_url').eq('id',currentInvitationId).eq('owner_id',currentUser.id).maybeSingle();if(old.error)throw old.error;if(old.data&&old.data.content&&typeof old.data.content==='object')existing=old.data.content}var premium={};if(typeof window.invellaBeforeCloudSave==='function'){premium=await window.invellaBeforeCloudSave({id:currentInvitationId,slug:currentInvitationSlug})||{}}var standard=collectData(),premiumContent=premium.content&&typeof premium.content==='object'?premium.content:premium,data=Object.assign({},existing,standard,premiumContent),payload={owner_id:currentUser.id,title:$('names').value||'Новое приглашение',event_date:eventISO(),venue_name:$('venue').value,venue_address:$('address').value,template_key:template,content:data,design:{colors:[data.c1,data.c2,data.c3,data.c4]}};if(premium.cover_url)payload.cover_url=premium.cover_url;if(currentInvitationId){var u=await sb.from('invitations').update(payload).eq('id',currentInvitationId).eq('owner_id',currentUser.id).select('id,slug').maybeSingle();if(u.error)throw u.error;if(u.data){currentInvitationSlug=u.data.slug;if(typeof window.invellaAfterCloudSave==='function')await window.invellaAfterCloudSave({id:currentInvitationId,slug:currentInvitationSlug});return true}currentInvitationId=null;currentInvitationSlug=null}payload.slug=makeSlug();var r=await sb.from('invitations').insert(payload).select('id,slug').single();if(r.error)throw r.error;currentInvitationId=r.data.id;currentInvitationSlug=r.data.slug;safeSet('invella_cloud_id_'+currentUser.id,r.data.id);safeSet('invella_cloud_slug_'+currentUser.id,r.data.slug);if(typeof window.invellaAfterCloudSave==='function'){var more=await window.invellaAfterCloudSave({id:currentInvitationId,slug:currentInvitationSlug});if(more){var finalPremium=await window.invellaBeforeCloudSave({id:currentInvitationId,slug:currentInvitationSlug})||{},finalContent=finalPremium.content&&typeof finalPremium.content==='object'?finalPremium.content:finalPremium,finalPayload={content:Object.assign({},data,finalContent)};if(finalPremium.cover_url)finalPayload.cover_url=finalPremium.cover_url;var f=await sb.from('invitations').update(finalPayload).eq('id',currentInvitationId).eq('owner_id',currentUser.id);if(f.error)throw f.error}}return true}catch(e){console.error('Invella save:',e);return false}}
var cloudSaveQueue=Promise.resolve();function cloudSave(){var run=cloudSaveQueue.then(performCloudSave,performCloudSave);cloudSaveQueue=run.catch(function(){});return run}window.invellaCloudSave=cloudSave;
...
function publicAssetUrl(u){return String(u||'').replace('https://lwanymjmbcstvggmhevx.supabase.co/',location.origin+'/sb/')}
function renderPublicMedia(x){if(String(x.plan||'').toLowerCase()!=='premium')return;var c=x.content||{},inv=document.querySelector('.invitation');if(!inv)return;var photos=Array.isArray(c.premiumGallery)?c.premiumGallery:[];if(photos.length&&!document.getElementById('invPublicGallery')){var g=document.createElement('div');g.id='invPublicGallery';g.className='block';var title=document.createElement('div');title.className='eyebrow';title.textContent='ФОТОГАЛЕРЕЯ';g.appendChild(title);if(c.premiumGalleryText){var caption=document.createElement('p');caption.textContent=c.premiumGalleryText;g.appendChild(caption)}var grid=document.createElement('div');grid.style.cssText='display:grid;grid-template-columns:repeat(2,1fr);gap:8px';photos.slice(0,8).forEach(function(u){var img=document.createElement('img');img.src=publicAssetUrl(u);img.loading='lazy';img.decoding='async';img.style.cssText='width:100%;height:170px;object-fit:cover;border-radius:14px';grid.appendChild(img)});g.appendChild(grid);inv.appendChild(g)}if(c.premiumMusicUrl&&!document.getElementById('invPublicMusicBtn')){var audio=document.createElement('audio');audio.id='invPublicAudio';audio.preload='none';audio.style.display='none';var musicSource=publicAssetUrl(c.premiumMusicUrl);var start=Math.max(0,Number(c.premiumMusicStart)||0),end=Math.max(0,Number(c.premiumMusicEnd)||0);audio.addEventListener('loadedmetadata',function(){if(start)audio.currentTime=start});audio.addEventListener('timeupdate',function(){if(end&&audio.currentTime>=end){audio.pause();audio.currentTime=start}});var button=document.createElement('button');button.id='invPublicMusicBtn';button.type='button';button.textContent='♪';button.setAttribute('aria-label','Музыка');button.style.cssText='position:fixed;right:18px;bottom:18px;z-index:10000;width:50px;height:50px;border-radius:50%;font-size:24px';button.onclick=function(){if(audio.paused){if(!audio.getAttribute('src'))audio.src=musicSource;if(start&&audio.readyState>=1&&audio.currentTime<start)audio.currentTime=start;audio.play().catch(function(){})}else audio.pause()};document.body.appendChild(audio);document.body.appendChild(button)}}
...
$('photo').addEventListener('change',async function(e){var original=e.target.files[0];if(!original)return;var f=await compressImage(original);var r=new FileReader();r.onload=function(){$('invHero').style.backgroundImage='linear-gradient(rgba(0,0,0,.28),rgba(0,0,0,.28)),url("'+r.result+'")';$('invHero').style.color='#fff'};r.readAsDataURL(f);/* The explicit save uploads the selected cover once. */});
