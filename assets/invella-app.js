...
var currentCoverPath='',coverPreviewUrl='',currentMusicPath='',musicPreviewUrl='',currentGalleryPaths=[],galleryPreviewUrls=[],countdownTimer=null,pendingCoverFile=null,pendingMusicFile=null,pendingGalleryFiles=[];
...
function enforceEditorPlan(){var paid=currentEditorPlan&&currentEditorPlan!=='draft';var pro=editorHas('pro'),premium=editorHas('premium');if(paid&&!pro){if($('backgroundStyle'))$('backgroundStyle').value='auto';if($('showRSVP'))$('showRSVP').checked=false}if(paid&&!premium&&$('backgroundStyle')&&$('backgroundStyle').value==='custom')$('backgroundStyle').value=pro?'silk':'auto';document.querySelectorAll('#bgPicker .bg-choice').forEach(function(b){var need=b.dataset.bg==='auto'?'basic':'pro';b.disabled=paid&&!editorHas(need);b.classList.toggle('plan-locked',b.disabled)});if($('customBackgroundFile'))$('customBackgroundFile').disabled=paid&&!premium;if($('photo'))$('photo').disabled=paid&&!pro;if($('guestNamesForLinks'))$('guestNamesForLinks').disabled=paid&&!pro;if($('invFont'))$('invFont').disabled=paid&&!pro;var gl=$('guestNamesForLinks')&&$('guestNamesForLinks').closest('.feature-box');if(gl){var b=gl.querySelector('button');if(b)b.disabled=paid&&!pro}if($('showRSVP'))$('showRSVP').disabled=paid&&!pro;['premiumMusic','premiumGallery','premiumTimer'].forEach(function(id){if($(id))$(id).disabled=paid&&!premium})}
...
window.invellaSavePublished=async function(){var b=document.querySelector('.appbar .toolbar .primary');if(!currentInvitationId)return alert('Приглашение не выбрано.');if(b){b.disabled=true;b.textContent='Сохраняем…'}try{var ok=await cloudSave();if(!ok)throw new Error('Не удалось обновить запись.');alert('Изменения сохранены')}catch(e){alert(e.message||'Не удалось сохранить изменения.')}finally{if(b){b.disabled=false;b.textContent='Сохранить изменения'}}}
...
$('photo').addEventListener('change',async function(e){var original=e.target.files[0];if(!original)return;pendingCoverFile=original;coverPreviewUrl=URL.createObjectURL(original);applyTemplate();var f=await compressImage(original);if(currentUser){var saved=await cloudSave();if(saved){await uploadCover(f);await cloudSave();pendingCoverFile=null}}});
if($('premiumTimer'))$('premiumTimer').addEventListener('change',function(){renderPremiumMedia();save()});
if($('premiumMusic'))$('premiumMusic').addEventListener('change',async function(e){var f=e.target.files&&e.target.files[0];if(!f)return;pendingMusicFile=f;musicPreviewUrl=URL.createObjectURL(f);renderPremiumMedia();if(currentUser){var saved=await cloudSave();if(saved){currentMusicPath=await uploadAsset(f,'music');musicPreviewUrl=await signedAssetUrl(currentMusicPath);await cloudSave();pendingMusicFile=null;renderPremiumMedia()}}});
if($('premiumGallery'))$('premiumGallery').addEventListener('change',async function(e){var fs=Array.from(e.target.files||[]).slice(0,5);pendingGalleryFiles=fs;galleryPreviewUrls=fs.map(function(f){return URL.createObjectURL(f)});renderPremiumMedia();if(currentUser&&fs.length){var saved=await cloudSave();if(saved){var paths=[];for(var i=0;i<fs.length;i++)paths.push(await uploadAsset(await compressImage(fs[i]),'gallery',i));currentGalleryPaths=paths;galleryPreviewUrls=(await Promise.all(paths.map(signedAssetUrl))).filter(Boolean);await cloudSave();pendingGalleryFiles=[];renderPremiumMedia()}}});
